...
Contribution to `capuchin_crypt` is encouraged: bug reports, discussions,
...
`capuchin_crypt` is governed under the
...
- All code changes require tests. In most cases, this will be added or updated
  unit tests. I use [gleeunit][gleeunit].

- I use the Gleam code formatter and [glinter][glinter] for linting to ensure
...
`capuchin_crypt` contributions must be well understood by the submitter and
...
`capuchin_crypt` has adopted a variation of the Conventional Commits format
...
Austin Ziegler created `capuchin_crypt`.
